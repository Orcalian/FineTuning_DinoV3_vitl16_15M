...
def init_fsdp_model_from_checkpoint(
    ...
    if not Path(checkpoint_path).is_dir():  # PyTorch standard checkpoint
        ...
        # Init depuis backbone pre-entraine : les tetes (dino_head/ibot_head) et
        # local_cls_norm sont initialisees from scratch -> manquantes attendues.
        if missing:
            print(f"[init backbone] {len(missing)} cles manquantes (tetes from scratch), ex:", missing[:4])
        if unexpected:
            print(f"[init backbone] {len(unexpected)} cles inattendues, ex:", unexpected[:4])
    else:  # DCP checkpoint
